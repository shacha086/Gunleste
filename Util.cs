        ...
        public static Dictionary<Type, Action<Entity, Player, Bullet>> DamageableEntities { get; } = new() {
            {
                    ...
                    if (count > 3) {
                        ...
                        entity.Scene.Add(
                            Engine.Pooler.Create<Debris>()
                                .Init(
                                    new Vector2(
                                        bullet.Facing == Facings.Left ? bullet.Center.X + 2 : bullet.Center.X - 2,
                                        bullet.Center.Y
                                    ), tileType
                                )
                ...
                typeof(DashSwitch), (entity, _, ballet) => {
                    ...
                    void HandleDashSwitch(Vector2 direction) {
                        ...
                        entity.SceneAs<Level>()
                            .ParticlesFG.Emit(
                                switchData.Get<bool>("mirrorMode") ? DashSwitch.P_PressAMirror : DashSwitch.P_PressA,
                                10,
                ...
                typeof(Booster), (entity, _, bullet) => {
                    ...
                    Audio.Play(
                        ...
                        boosterData.Get<Sprite>("sprite").RenderPosition
                    );
                ...
                typeof(CrushBlock), (entity, _, bullet) => {
                    if (entity is not CrushBlock block) {
                        return;
                    }

                    var blockData = DynamicData.For(block);
                    var direction = bullet.Direction;
                    if (!blockData.Invoke<bool>("CanActivate", direction)) {
                        return;
                    }

                    blockData.Invoke("Attack", -direction);
                    bullet.RemoveSelf();
                }
            }, {
                typeof(FallingBlock), (entity, _, bullet) => {
                    if (entity is not FallingBlock block) {
                        return;
                    }
                ...
                typeof(SwapBlock), (entity, _, bullet) => {
                    ...
                    var swapBlocks = entity.Scene.Tracker.GetEntities<SwapBlock>();
                    foreach (var it in swapBlocks) {
                        var blockData = DynamicData.For(it);
                        blockData.Invoke("OnDash", bullet.Direction);
                    }
                }
            }, {
                typeof(FlyFeather), (entity, player, bullet) => {
                    if (entity is not FlyFeather feather) {
                        return;
                    }

                    var featherData = DynamicData.For(feather);
                    featherData.Invoke("OnPlayer", player);
                    bullet.RemoveSelf();
                }
            }, {
